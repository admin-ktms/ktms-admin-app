...
summary.innerHTML = "<div class="ktms-error-state"> <strong>Unable to load registrations</strong> <p>${escapeHtml(error.message)}</p> </div>";
}
}
...
registrationStatuses: Array.isArray(
  data?.registrationStatuses
)
  ? data.registrationStatuses
  : [],

paymentStatuses: Array.isArray(
...
select.innerHTML = "<option value="">All tournament types</option> ${options}";
...
select.innerHTML = "<option value="">All tournaments</option> ${options}";

select.value = selected || "";
}

function populateStatusFilters(data) {
const registrationStatuses = Array.isArray(
data?.registrationStatuses
)
? data.registrationStatuses
: [];

const paymentStatuses = Array.isArray(
data?.paymentStatuses
)
? data.paymentStatuses
: [];
...
registrationStatusSelect.innerHTML = `
<option value="">
All statuses
</option>
...
return "<span class="ktms-status ktms-status-${statusClass(status)}"> ${escapeHtml(status)} </span>";
}
...
return "<div class="ktms-empty-state"> <h3>${escapeHtml(title)}</h3> <p>${escapeHtml(message)}</p> </div>";
}
